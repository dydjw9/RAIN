...
from .generate_gradient import cw_grad,pgd_grad
import copy
sys.path.append("../")
...
class BlackBoxL2:
    ...
    def __init__(self, epsilon,learning_rate,num_classes,targeted = False, search_steps=None, max_steps=None, cuda=True, debug=True,isnorm = False,iszoo=False,isstl10=False):
        self.debug = debug
        ...
        if self.iszoo:
            self.zoo = generate_gradient(torch.device("cuda"),targeted=self.targeted, classes = num_classes)
    ...
    def _optimize(self, model, meta_model, step, input_var, modifier_var, target_var, scale_const_var, target, batch_idx, indice, input_orig = None):
        ...
        if not self.iszoo:
            ...
            meta_output ,indice2= self.zoo.run(model,input_adv,target,indice)
            indice = indice2
