...
class embed_resnet(nn.Module):
    def __init__(self,edsr_model,dnn_model,RdmDU=True,gaus_noise=False,edsr_out=False,mean_filter=args.ismean_filter):
        super(embed_resnet,self).__init__()
        self.edsr_model = edsr_model
        self.dnn_model = dnn_model
        self.normVector = {'mean':torch.from_numpy(np.array([0.485, 0.456, 0.406])), 
                                'std':torch.from_numpy(np.array([0.229, 0.224, 0.225]))}
        # whether to take random downsampling or not 
        self.isRdmDU = RdmDU
        self.edsr_out = edsr_out
    ...
    def forward(self,x):
        ...
        x = edsr_output
        #The output of EDSR is 0-255, so need to normalize them 
        x = self.normalize(x)
    ...
    def normalize(self,img):
        ...
        img = img/255
        mean = self.normVector["mean"].view(1,3,1,1).float().cuda()
        std = self.normVector["mean"].view(1,3,1,1).float().cuda()
        for i in range(3):
    ...
    def denorm(self,img):
        ...
        std = self.normVector["mean"].view(1,3,1,1).float().cuda()
        for i in range(3):
            img[:,i,:,:] = (img[:,i,:,:]*self.normVector['std'][i]+self.normVector['mean'][i]) 
        img = img * 255
        img = torch.round(img)
        img = torch.clamp(img,0,255)
        return img
