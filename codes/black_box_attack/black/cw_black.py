...
from .generate_gradient import cw_grad,pgd_grad,nes_grad
import copy
sys.path.append("../")
...
class BlackBoxL2:
    ...
    def __init__(self, epsilon,learning_rate,num_classes,targeted = False, search_steps=None, max_steps=None, cuda=True, debug=True,isnorm = False):
        self.debug = debug
    ...
    def _optimize(self, model, meta_model, step, input_var, modifier_var, target_var, scale_const_var, target, batch_idx, indice, input_orig = None):
        ...
        # meta_output = pgd_grad(model,input_adv,target,self.num_classes,self.targeted)
        meta_output = NES_Grad.run(model,input_adv,target,self.num_classes,self.targeted)
        ...
        '''
        if step == 0:
            tmp = copy.deepcopy(input_var)
            tmp_grad = meta_model(tmp).reshape(-1)
        
            scio.savemat('./grad' + str(batch_idx)+'.mat', {'grad': np.sort(abs(tmp_grad.data.cpu().numpy()))})
            #pdb.set_trace()
            #indice = self.var_list
        '''
    ...
    def run(self, model, meta_model, input, target, batch_idx = 0):
        ...
        if self.isnorm:
            ...
            up_bound = torch.ones_like(input) * 255
            down_bound = torch.zeros_like(input)
        ...
        for search_step in range(self.binary_search_steps):
            ...
            for step in range(self.max_steps):
                ...
                for i in range(batch_size):
                    # the result is for last iteration results 
